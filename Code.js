...
function doGet(e) {
  // Check for page parameter to serve different pages
  const page = e.parameter.page;
  
  if (page === 'performance') {
    return HtmlService.createHtmlOutputFromFile("performance-test-ui")
      .setTitle("NAMS Performance Test Dashboard")
...
/**
 * Quick performance test for the main API endpoint
 */
function api_quickPerformanceTest() {
  console.log('=== Quick Performance Test ===');
  
  const startTime = Date.now();
  
  try {
    const result = api_getCachedStudentData();
    ...
    return {
      ...
      studentCount: result.metadata?.studentCount,
      target: '3000ms',
...
/**
 * Quick performance check for the performance test dashboard
 */
...
function testCachingLayer() {
  ...
  try {
    ...
    const setResult = cacheManager.set('cache_test_1', testData, 5);
    const getData = cacheManager.get('cache_test_1');
    
    if (setResult && getData && getData.message === testData.message) {
      results.passed++;
      results.tests.push({ name: 'Set/Get Test', status: 'PASSED' });
    } else {
      results.failed++;
      results.tests.push({ name: 'Set/Get Test', status: 'FAILED', error: 'Data mismatch' });
    }
    
    // Test 2: TTL validation
    const isValid = cacheManager.isValid('cache_test_1');
    if (isValid) {
      results.passed++;
      results.tests.push({ name: 'TTL Validation', status: 'PASSED' });
    } else {
      results.failed++;
      results.tests.push({ name: 'TTL Validation', status: 'FAILED' });
    }
    
    // Test 3: CachedRosterService
    const rosterService = new CachedRosterService();
    const cacheInfo = rosterService.getCacheInfo();
    
    if (cacheInfo) {
      results.passed++;
      results.tests.push({ name: 'Roster Service', status: 'PASSED' });
    } else {
      results.failed++;
      results.tests.push({ name: 'Roster Service', status: 'FAILED' });
    }
    
    // Cleanup
    cacheManager.invalidate('cache_test_1');
    
  } catch (error) {
    results.failed++;
    results.tests.push({ name: 'Caching Layer', status: 'FAILED', error: error.message });
  }
...
function generatePerformanceReport() {
  ...
  try {
    ...
    report.performance = {
      ...
      apiCalls: perfMetrics.apiCalls,
      timing: perfMetrics.timing,
      slowOperations: perfMetrics.slowOperations.length
    };
    ...
    const cacheInfo = rosterService.getCacheInfo();
    report.caching = {
      rosterCache: cacheInfo.roster ? 'Available' : 'Not Available',
      statusCache: cacheInfo.status ? 'Available' : 'Not Available',
      metrics: cacheInfo.metrics
    };

    // Generate recommendations
    if (perfMetrics.timing.averageResponseTime > 2000) {
      report.recommendations.push('Consider optimizing slow operations');
    }
    
    if (perfMetrics.apiCalls.cacheHitRate < 50) {
      report.recommendations.push('Improve cache hit rate by adjusting TTL settings');
    }
    
    if (perfMetrics.slowOperations.length > 5) {
      report.recommendations.push('Review and optimize frequently slow operations');
    }
...
function testEndToEndPerformance() {
  ...
  try {
    ...
    const studentData = api_getCachedStudentData();
    const loadTime = Date.now() - loadStart;
    
    results.tests.push({
      name: 'Data Loading',
      duration: loadTime,
      passed: loadTime < 3000 && studentData.success,
      target: '< 3000ms',
      details: `Loaded ${studentData.data?.students?.length || 0} students`
    });
    ...
    results.tests.push({
      ...
      duration: cacheTime,
      passed: cacheTime < 1000,
      target: '< 1000ms',
      details: `Cache ${cachedData.metadata?.cacheUsed ? 'HIT' : 'MISS'}`
    });
 ...
 */
function testSingleEndpoint(endpointName) {
  ...
  try {
    const func = eval(endpointName);
    if (typeof func !== 'function') {
      throw new Error(`${endpointName} is not a function`);
    }

    const testResult = func();
    result.success = true;
...
function runAllPerformanceTests() {
  ...
  try {
    // Run quick check
    suite.tests.quick = quickPerformanceCheck();
    
    // Run caching tests
    suite.tests.caching = testCachingLayer();
    
    // Run end-to-end tests
    suite.tests.endToEnd = testEndToEndPerformance();
    
    // Determine overall result
    suite.overallPassed = 
      suite.tests.quick.overallPassed &&
      suite.tests.caching.overallPassed &&
      suite.tests.endToEnd.overallPassed;
